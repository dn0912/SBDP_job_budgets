...
async function fetchXRayTracePeriodically(dateNow, jobId) {
...
async function fetchTraceFromRedisPeriodically(dateNow, jobId) {
  // TODO: to measure trace fetching delay
  const pollPeriodinMs = 100
  const counter = {
    value: 0,
  }
  ...
  while (counter.value < 60) {
    // eslint-disable-next-line no-await-in-loop
    await new Promise((resolve) => setTimeout(resolve, pollPeriodinMs))
    // eslint-disable-next-line no-await-in-loop
    const traceCloseTimeStampAnnotation = await redisClient.get(jobId)

    const currentTimeStamp = moment.utc().valueOf()
    console.log('+++currentTimeStamp', currentTimeStamp, traceCloseTimeStampAnnotation)
    ...
    // TODO: remove break statement => only for first fetch to record trace delay
    counter.value++
    if (traceCloseTimeStampAnnotation) {
      console.log('+++traceSummaryArray', traceResult, { pollPeriodinMs })
      // fs.appendFileSync(
      //   'traceFetchingDelays.csv',
      //   `\n${traceResult.jobStartTimeStamp}, ${traceResult.arn}, ${traceResult.traceCloseTimeStamp}, ${traceResult.currentTimeStamp}, ${traceResult.elapsedTimeFromClosingTraceToNow}`,
...
const startTracing = async (req, res) => {
  ...
  // fetchXRayTracePeriodically(dateNow, jobId)
  fetchTraceFromRedisPeriodically(dateNow, jobId)
